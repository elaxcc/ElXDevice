...
ElMcDevice::ElMcDevice() : ElXDevice()
{
	ResizeBuffer(512);
	ReInitParse();
}

ElMcDevice::ElMcDevice(int BufferSize) : ElXDevice()
{
	ResizeBuffer(BufferSize);
	ReInitParse();
}

ElMcDevice::ElMcDevice(std::string _PortName, int _BaudRate, int BufferSize) : ElXDevice(_PortName, _BaudRate)
{
	ResizeBuffer(BufferSize);
	ReInitParse();
}

ElMcDevice::~ElMcDevice()
{
}

void ElMcDevice::ReInitParse() {
...
int ElMcDevice::Parse(unsigned char *Data, int DataSize) {
						...
						else
						{
							...
							if (!(ParseFlag & PARSE_FALG_CRCH))
							{
								...
								if (ParseCRCInCm == ParseCRCCalc)
								{
									...
									// Process packet
									err = ProcessPacket(ParseFC, ParseBuf, ParseBC - 3);
								}
								else
								{
...
int ElMcDevice::CreatePacket(unsigned char FuncCode, unsigned char *Data, unsigned short DataSize, unsigned char *Buffer) {
...
int ElMcDevice::CalcCRC16(unsigned char *Data, unsigned short DataSize, unsigned short CurrentValCRC16) {
	for (int i = 0; i < DataSize; i++)
	{
